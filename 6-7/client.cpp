...
void start_dinner(int philosopher_id, int socket, int viewer_socket) {
  ...
  while (count_of_meals < 5) {
    ...
    if (current_state == 0) {
      ...
      sprintf(buff, "Philosopher %d: is thinking\n", philosopher_id);
      ...
      if (result == 0) {
        ...
        sprintf(buff, "Philosopher %d: Takes two forks and eating\n", philosopher_id);
        if (send(viewer_socket, buff, sizeof buff, 0) == -1) {
          perror("Error sending message to viewer");
          exit(1);
        }
      ...
      sprintf(buff, "Philosopher %d: Giving back two forks\n", philosopher_id);
      if (send(viewer_socket, buff, sizeof buff, 0) == -1) {
        perror("Error sending message to viewer");
        exit(1);
      }
...
int main() {
  srand(time(NULL));
  int client_socket, viewer_socket;
  int port;
  char address[1024];
  struct sockaddr_in server_addr, viewer_addr;
  ...
  start_dinner(id, client_socket, viewer_socket);
